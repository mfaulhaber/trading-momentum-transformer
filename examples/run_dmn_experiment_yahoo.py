...
import pandas as pd
from settings.default import YAHOO_TICKERS
...
ASSET_CLASS_MAPPING = dict(zip(YAHOO_TICKERS, ["COMB"] * len(YAHOO_TICKERS)))
TRAIN_VALID_RATIO = 0.90
TIME_FEATURES = False
FORCE_OUTPUT_SHARPE_LENGTH = None
EVALUATE_DIVERSIFIED_VAL_SHARPE = True
NAME = f"experiment_yahoo_{len(YAHOO_TICKERS)}assets"
    ...
    for v in versions:
        ...
        if changepoint_lbws:
            features_file_path = os.path.join(
                "data",
                f"yahoo_cpd_{np.max(changepoint_lbws)}lbw.csv",
            )
            print(f"Using {features_file_path}")
        else:
            features_file_path = os.path.join(
                "data",
                "yahoo_cpd_nonelbw.csv",
            )
