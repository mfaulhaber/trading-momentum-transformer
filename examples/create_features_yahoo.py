...
from data.pull_data import pull_yahoo_data
from settings.default import (
    YAHOO_TICKERS,
    CPD_YAHOO_OUTPUT_FOLDER,
    FEATURES_YAHOO_FILE_PATH,
)
...
def main(
    ...
    features = pd.concat(
        [
            deep_momentum_strategy_features(pull_yahoo_data(ticker)).assign(
                ticker=ticker
            )
            ...
            for extra in extra_lbw:
                extra_data = pd.read_csv(
                    output_file_path.replace(
                        f"yahoo_cpd_{lookback_window_length}lbw.csv",
                        f"yahoo_cpd_{extra}lbw.csv",
                    ),
                    index_col=0,
                    parse_dates=True,
                ).reset_index()[
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        #     type=str,
        #     nargs="?",
        #     default=CPD_YAHOO_OUTPUT_FOLDER_DEFAULT,
        #     # choices=[],
        ...
        #     type=str,
        #     nargs="?",
        #     default=FEATURES_YAHOO_FILE_PATH_DEFAULT,
        #     # choices=[],
        ...
        return (
            YAHOO_TICKERS,
            CPD_YAHOO_OUTPUT_FOLDER(args.lookback_window_length),
            args.lookback_window_length,
            FEATURES_YAHOO_FILE_PATH(args.lookback_window_length),
            args.extra_lbw,
        )
